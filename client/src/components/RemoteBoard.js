...
function RemoteBoard({room, user}) {
  ...
  useEffect(() => {
    ...
    socketRef.current.emit('join room', {'room': roomRef.current, 'user': user});

    socketRef.current.on('player connection', (data) => {
      if(data.user !== user){
        alert(data.user + ' has joined')
      }
    });

    socketRef.current.on('player move', (newBoard) => {
  ...
  return (
    ...
    <div>
      <p>Hello: {user}</p>
      <p>Share your room: {roomRef.current}</p>
    </div>
    <div>
    {board.map((rows, i) =>(
      <div key={"r-" + i}
        ...
        <div 
          ...
          onClick={() => {
            ...
            if(bufferState === -1 || bufferState === 0){
              ...
              socketRef.current.emit('room move', room, newBoard);
              setBufferState(-1);
              setBufferTile([-1,-1]);
              //setBoard(newBoard);
